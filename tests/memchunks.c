...
static xuint_t mem_chunk_recursion = 0;
...
struct _GMemChunk
{
  const xchar_t *name;         /* name of this MemChunk...used for debugging output */
  xint_t type;                 /* the type of MemChunk: ALLOC_ONLY or ALLOC_AND_FREE */
...
old_mem_chunk_alloc (GMemChunk *mem_chunk)
{
   ...
   */
  mem = (xpointer_t) &mem_chunk->mem_area->mem[mem_chunk->mem_area->index];
