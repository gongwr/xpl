...
g_utf8_find_basename (const xchar_t *filename,
                      gssize       len)
{
                             ...
                             xchar_t           **dll_function_out)
{
  ...
  if (ex_out)
    {
      if (ex_basename_out)
        *ex_basename_out = (xchar_t *) g_utf8_find_basename (ex, -1);
  ...
  if (ex_folded_out)
    {
      if (ex_folded_basename_out)
        *ex_folded_basename_out = (xchar_t *) g_utf8_find_basename (ex_folded, -1);
