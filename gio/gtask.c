 ...
 *
 * The most common usage of #GTask is as a #xasync_result_t, to
 ...
 *
 * Here is an example for using GTask as a xasync_result_t:
 * |[<!-- language="C" -->
 *     typedef struct {
 ...
 * #xcancellable_t, #GMainContext, and [I/O priority][io-priority]
...
struct _GTask {
  ...
  xchar_t *name;  /* (owned); may only be modified before the #GTask is threaded */
...
G_DEFINE_TYPE_WITH_CODE (GTask, g_task, XTYPE_OBJECT,
...
static void
g_task_finalize (xobject_t *object)
{
 ...
 */
GTask *
g_task_new (xpointer_t              source_object,
            xcancellable_t         *cancellable,
            xasync_ready_callback_t   callback,
            xpointer_t              callback_data)
{
  ...
  task = g_object_new (XTYPE_TASK, NULL);
 ...
 */
void
g_task_report_error (xpointer_t             source_object,
                     xasync_ready_callback_t  callback,
                     xpointer_t             callback_data,
                     xpointer_t             source_tag,
                     xerror_t              *error)
 ...
 */
void
g_task_report_new_error (xpointer_t             source_object,
                         xasync_ready_callback_t  callback,
                         xpointer_t             callback_data,
                         xpointer_t             source_tag,
                         GQuark               domain,
 ...
 * g_task_had_error() will check the task's #xcancellable_t first, and
...
static xobject_t *
g_task_ref_source_object (xasync_result_t *res)
{
...
static void
g_task_thread_pool_thread (xpointer_t thread_data,
  ...
  if (task->cancellable)
    {
       ...
       * xcancellable_t, but is necessary to avoid a race on finalising the GTask
 ...
 * #xasync_ready_callback_t will be invoked in @task's #GMainContext.
 *
 ...
 *
 * If @result is %NULL then a #GValue of type %XTYPE_POINTER
                     ...
                     GValue *result)
{
  ...
  if (result == NULL)
    {
      g_value_init (value, XTYPE_POINTER);
 ...
 */
xboolean_t
g_task_is_valid (xpointer_t result,
                 xpointer_t source_object)
{
  if (!X_IS_TASK (result))
    return FALSE;
...
static void
g_task_get_property (xobject_t    *object,
                     xuint_t       prop_id,
                     GValue     *value,
                     GParamSpec *pspec)
{
...
static xpointer_t
g_task_get_user_data (xasync_result_t *res)
{
  return G_TASK (res)->callback_data;
}

static xboolean_t
g_task_is_tagged (xasync_result_t *res,
                  xpointer_t      source_tag)
{
