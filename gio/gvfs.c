...
G_DEFINE_TYPE_WITH_PRIVATE (GVfs, g_vfs, XTYPE_OBJECT)

static void
g_vfs_dispose (xobject_t *object)
{
...
static void
g_vfs_uri_lookup_func_closure_free (xpointer_t data)
{
                     ...
                     const char *parse_name)
{
  ...
  while (g_hash_table_iter_next (&iter, NULL, (xpointer_t *) &closure))
    {
  ...
  if (!priv->supported_schemes)
    {
      ...
      for (; default_schemes && *default_schemes; default_schemes++)
        g_ptr_array_add (supported_schemes, (xpointer_t) *default_schemes);
      ...
      while (g_hash_table_iter_next
        ...
        g_ptr_array_add (supported_schemes, (xpointer_t) additional_scheme);
  ...
  if (g_once_init_enter (&vfs_default_singleton))
    {
      ...
      singleton = _xio_module_get_default (G_VFS_EXTENSION_POINT_NAME,
                                            "GIO_USE_VFS",
                                            (xio_module_verify_func_t) g_vfs_is_active);
