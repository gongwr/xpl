...
#define XTYPE_FILE_IO_STREAM         (g_file_io_stream_get_type ())
#define G_FILE_IO_STREAM(o)           (XTYPE_CHECK_INSTANCE_CAST ((o), XTYPE_FILE_IO_STREAM, GFileIOStream))
#define G_FILE_IO_STREAM_CLASS(k)     (XTYPE_CHECK_CLASS_CAST((k), XTYPE_FILE_IO_STREAM, GFileIOStreamClass))
...
#define G_FILE_IO_STREAM_GET_CLASS(o) (XTYPE_INSTANCE_GET_CLASS ((o), XTYPE_FILE_IO_STREAM, GFileIOStreamClass))
...
struct _GFileIOStreamClass
{
  ...
  xboolean_t    (* can_seek)          (GFileIOStream    *stream);
  xboolean_t    (* seek)	            (GFileIOStream    *stream,
                                     goffset               offset,
                                     GSeekType             type,
                                     xcancellable_t         *cancellable,
                                     xerror_t              **error);
  xboolean_t    (* can_truncate)      (GFileIOStream    *stream);
  xboolean_t    (* truncate_fn)       (GFileIOStream    *stream,
                                     goffset               size,
                                     xcancellable_t         *cancellable,
                                     xerror_t              **error);
...
xtype_t      g_file_io_stream_get_type          (void) G_GNUC_CONST;
