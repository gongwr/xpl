...
struct _GDBusServerClass
{
  ...
  /*< public >*/
  /* Signals */
  xboolean_t (*new_connection) (GDBusServer      *server,
...
G_DEFINE_TYPE_WITH_CODE (GDBusServer, g_dbus_server, XTYPE_OBJECT,
                        ...
                        xerror_t            **error)
{
  ...
  server = g_initable_new (XTYPE_DBUS_SERVER,
                           cancellable,
                           error,
                           "address", address,
                           "flags", flags,
  ...
  if (server->nonce != NULL)
    {
      ...
      if (!g_input_stream_read_all (g_io_stream_get_input_stream (XIO_STREAM (socket_connection)),
                                    buf,
                                    16,
                                    &bytes_read,
                                    NULL,  /* xcancellable_t */
               ...
               xerror_t       **error)
{
  ...
  server->listener = XSOCKET_LISTENER (g_threaded_socket_service_new (-1));
