...
{
  GHashTable   *networks  /* (element-type xinet_address_mask_t) (owned) */;
...
static xuint_t inet_address_mask_hash (gconstpointer key);
static xboolean_t inet_address_mask_equal (gconstpointer a,
                                         gconstpointer b);
                             ...
                             xpointer_t      user_data)
{
  ...
  xnetwork_monitor_base_t *base = g_task_get_source_object (task);
  xsocket_address_t *addr;
  xerror_t *error = NULL;
...
inet_address_mask_hash (gconstpointer key)
{
  ...
  const guint8 *bytes = xinet_address_to_bytes (addr);
