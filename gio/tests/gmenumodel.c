  ...
  for (i = 0; i < n; i++)
    {
      ...
      while (g_menu_attribute_iter_next (attr_iter))
        {
          ...
          xvariant_t *value = g_menu_attribute_iter_get_value (attr_iter);
      ...
      while (g_menu_link_iter_next (link_iter))
        {
          const xchar_t *name = g_menu_link_iter_get_name (link_iter);
...
static RandomMenu * random_menu_new (GRand *rand, xint_t order);
