                    ...
                    xpointer_t               user_data)
{
  if (g_strcmp0 (method_name, "HelloWorld") == 0)
  ...
  else if (g_strcmp0 (method_name, "TestArrayOfPrimitiveTypes") == 0)
    {
      ...
      for (j = 0; j < 2; j++)
        for (i = 0; i < n_elts; i++)
          g_variant_builder_add (&ret, "b", (xboolean_t)bytes[i]);
      g_variant_builder_close (&ret);
      g_variant_unref (v);
      ...
      doubles = g_variant_get_fixed_array (v, &n_elts, sizeof (xdouble_t));
  ...
  else if (g_strcmp0 (method_name, "Sleep") == 0)
    {
      ...
      g_timeout_add ((xuint_t)msec, end_sleep, g_object_ref (invocation));
    }
