...
list_model_get (GListModel *model,
                xuint_t       position)
{
  xobject_t *item = g_list_model_get_item (model, position);
  xobject_t *object = g_list_model_get_object (model, position);
...
test_store_properties (void)
{
  ...
  store = g_list_store_new (XTYPE_MENU_ITEM);
...
/* Test that #GListStore rejects non-xobject_t item types. */
static void
  ...
  if (g_test_subprocess ())
    {
      ...
      g_object_new (XTYPE_LIST_STORE, "item-type", XTYPE_LONG, NULL);
      return;
    }
...
test_store_boundaries (void)
{
  ...
  g_list_store_splice (store, 0, 1, (xpointer_t *) &item, 1);
  g_assert_cmpuint (g_list_model_get_n_items (G_LIST_MODEL (store)), ==, 1);
  ...
  for (i = 0; i < n_items; i++)
    {
      ...
      g_object_add_weak_pointer (G_OBJECT (items[i]), (xpointer_t *) &items[i]);
               ...
               xpointer_t      user_data)
{
  ...
  xchar_t *a = g_object_get_data (a_o, "key");
  xchar_t *b = g_object_get_data (b_o, "key");
...
test_store_sorted (void)
{
  ...
  store = g_list_store_new (XTYPE_OBJECT);
...
test_store_splice_replace_middle (void)
{
  ...
  store = g_list_store_new (XTYPE_SIMPLE_ACTION);
...
test_store_splice_wrong_type (void)
{
  ...
  g_list_store_splice (store, 0, 0, (xpointer_t)&store, 1);
...
static xint_t
list_model_cmp_action_by_name (GAction *a, GAction *b, xpointer_t user_data)
{
  return g_strcmp0 (g_action_get_name (a), g_action_get_name (b));
}
...
test_store_signal_items_changed (void)
{
  ...
  g_assert_cmpuint (g_list_model_get_n_items (model), >=, 2);
  g_list_store_splice (store, 0, 2, (xpointer_t)&item, 1);
  g_object_unref (item);
  g_assert_true (expected.called);
