...
test_basic (void)
{
  ...
  g_object_add_weak_pointer (G_OBJECT (task), (xpointer_t *)&task);
  g_signal_connect (task, "notify::completed",
                    (GCallback) completed_cb, &notification_emitted);
...
xboolean_t wrong_thread_notification_emitted = FALSE;
...
static xpointer_t
wrong_thread_func (xpointer_t user_data)
{
  GTask *task = user_data;
...
test_run_in_thread (void)
{
  ...
  g_object_weak_ref (G_OBJECT (task), task_weak_notify, (xpointer_t)&weak_notify_ran);
  g_signal_connect (task, "notify::completed",
                    (GCallback) completed_cb, &notification_emitted);

  g_task_set_task_data (task, (xpointer_t)&thread_ran, NULL);
...
test_return_on_cancel (void)
{
  ...
  g_task_set_task_data (task, (xpointer_t)&thread_state, NULL);
  g_mutex_lock (&roc_init_mutex);
  g_mutex_lock (&roc_finish_mutex);
...
test_return_on_cancel_atomic (void)
{
  ...
  g_task_set_task_data (task, (xpointer_t)&state, NULL);
...
test_return_pointer (void)
{
  ...
  g_object_add_weak_pointer (object, (xpointer_t *)&object);
...
test_legacy_error (void)
{
  ...
  simple = g_simple_async_result_new (NULL, legacy_error_callback, &result,
                                      test_legacy_error);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_object_add_weak_pointer (G_OBJECT (simple), (xpointer_t *)&simple);
...
{
  xboolean_t error_first = GPOINTER_TO_UINT (g_task_get_task_data (task));
