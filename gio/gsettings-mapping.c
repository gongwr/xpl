                            ...
                            const xvariant_type_t *expected_type)
{
  ...
  if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_INT16))
    ...
    {
      if (0 <= l && l <= G_MAXUINT32)
        variant = g_variant_new_handle ((xuint_t) l);
    }
  else if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_DOUBLE))
    variant = g_variant_new_double ((xdouble_t) l);
                              ...
                              const xvariant_type_t *expected_type)
{
  ...
  if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_INT16))
    ...
    variant = g_variant_new_double ((xdouble_t) d);
                                     ...
                                     const xvariant_type_t *expected_type)
{
  ...
  if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_INT16))
    ...
    {
      if (u <= G_MAXINT32)
        variant = g_variant_new_int32 ((xint_t) u);
    }
  else if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_UINT32))
    {
      if (u <= G_MAXUINT32)
        variant = g_variant_new_uint32 ((xuint_t) u);
    }
  else if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_INT64))
    {
      if (u <= G_MAXINT64)
    ...
    {
      if (u <= G_MAXUINT32)
        variant = g_variant_new_handle ((xuint_t) u);
    }
  else if (g_variant_type_equal (expected_type, G_VARIANT_TYPE_DOUBLE))
    variant = g_variant_new_double ((xdouble_t) u);
  ...
  if (G_VALUE_HOLDS_BOOLEAN (value))
  ...
  else if (G_VALUE_HOLDS (value, XTYPE_STRV))
    {
      if (g_value_get_boxed (value) == NULL)
        return NULL;
      return g_variant_new_strv ((const xchar_t **) g_value_get_boxed (value),
                                 -1);
    }
