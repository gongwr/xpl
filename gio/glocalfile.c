...
G_DEFINE_TYPE_WITH_CODE (GLocalFile, g_local_file, XTYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (XTYPE_FILE,
						g_local_file_file_iface_init))

static char *find_mountpoint_for (const char *file, dev_t dev, xboolean_t resolve_basename_symlink);

#ifndef G_OS_WIN32
static xboolean_t is_remote_fs_type (const xchar_t *fsname);
#endif

static void
g_local_file_finalize (xobject_t *object)
{
  GLocalFile *local;
 ...
 *
 * Creates a #xfile_t from @dirname and @basename.
 *
 ...
 */
xfile_t *
g_local_file_new_from_dirname_and_basename (const xchar_t *dirname,
...
static xboolean_t
g_local_file_is_native (xfile_t *file)
{
  return TRUE;
}

static xboolean_t
g_local_file_has_uri_scheme (xfile_t      *file,
...
static char *
g_local_file_get_basename (xfile_t *file)
{
...
static char *
g_local_file_get_parse_name (xfile_t *file)
{
  const char *filename;
  char *parse_name;
...
static xfile_t *
g_local_file_get_parent (xfile_t *file)
{
...
static xuint_t
g_local_file_hash (xfile_t *file)
{
  GLocalFile *local = G_LOCAL_FILE (file);
...
static xboolean_t
g_local_file_equal (xfile_t *file1,
		    xfile_t *file2)
{
              ...
              const char *prefix)
{
  ...
  if (prefix_len > 0 &&
      X_IS_DIR_SEPARATOR (prefix[prefix_len-1]))
    prefix_len--;
...
static xboolean_t
g_local_file_prefix_matches (xfile_t *parent,
			     xfile_t *descendant)
{
...
static char *
g_local_file_get_relative_path (xfile_t *parent,
				xfile_t *descendant)
{
  GLocalFile *parent_local = G_LOCAL_FILE (parent);
  GLocalFile *descendant_local = G_LOCAL_FILE (descendant);
...
static xfile_t *
g_local_file_resolve_relative_path (xfile_t      *file,
...
static xfile_t *
g_local_file_get_child_for_display_name (xfile_t        *file,
...
static char *
find_mountpoint_for (const char *file, dev_t dev, xboolean_t resolve_basename_symlink)
{
  wchar_t *wpath;
  char *utf8_path;
...
static xfile_t *
g_local_file_set_display_name (xfile_t         *file,
			       const char    *display_name,
			       xcancellable_t  *cancellable,
			       xerror_t       **error)
{
		   ...
		   xerror_t       **error)
{
  ...
  if (fd == -1)
      ...
      if (errsv == EACCES)
	{
	  ...
	  ret = XPL_PRIVATE_CALL (g_win32_stat_utf8) (local->filename, &buf);
...
static xboolean_t
g_local_file_delete (xfile_t         *file,
		     xcancellable_t  *cancellable,
		     xerror_t       **error)
{
  GLocalFile *local = G_LOCAL_FILE (file);
...
find_mountpoint_for (const char *file,
                     dev_t       dev,
                     xboolean_t    resolve_basename_symlink)
{
  char *dir, *parent;
...
static xboolean_t
ignore_trash_path (const xchar_t *topdir)
{
  GUnixMountEntry *mount;
  xboolean_t retval = TRUE;
...
{
  static xsize_t home_dev_set = 0;
  static dev_t home_dev;
  static xboolean_t home_dev_valid = FALSE;
...
static xboolean_t
g_local_file_trash (xfile_t         *file,
		    xcancellable_t  *cancellable,
		    xerror_t       **error)
{
  GLocalFile *local = G_LOCAL_FILE (file);
...
static xboolean_t
g_local_file_make_directory (xfile_t         *file,
			     xcancellable_t  *cancellable,
			     xerror_t       **error)
{
  GLocalFile *local = G_LOCAL_FILE (file);
...
#ifdef HAVE_SYMLINK
static xboolean_t
g_local_file_make_symbolic_link (xfile_t         *file,
		   ...
		   xerror_t                **error)
{
  ...
  GStatBuf statbuf;
  xboolean_t destination_exist, source_is_dir;
  char *backup_name;
  int res;
  off_t source_size;
  xvfs_class_t *class;
...
xboolean_t
g_local_file_is_nfs_home (const xchar_t *filename)
{
  return FALSE;
}
...
static xboolean_t
is_remote_fs_type (const xchar_t *fsname)
{
  if (fsname != NULL)
    {
...
g_local_file_is_nfs_home (const xchar_t *filename)
{
  static xboolean_t remote_home = FALSE;
                                   ...
                                   xerror_t       **error)
{
  ...
  if (XPL_PRIVATE_CALL (g_win32_lstat_utf8) (name->data, &buf) != 0)
    {
      int errsv = errno;
      return g_local_file_measure_size_error (state->flags, errsv, name, error);
    }
