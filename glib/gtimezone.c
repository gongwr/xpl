...
#ifdef G_OS_UNIX
static GTimeZone *parse_footertz (const xchar_t *, size_t);
#endif
...
static void
zone_for_constant_offset (GTimeZone *gtz, const xchar_t *name)
{
  gint32 offset;
  TransitionInfo info;
...
zone_identifier_unix (void)
{
  ...
  if (resolved_identifier == NULL)
    {
      xboolean_t not_a_symlink = g_error_matches (read_link_err,
                                                G_FILE_ERROR,
                                                G_FILE_ERROR_INVAL);
  ...
  for (index = 0; index < type_count; index++)
    {
      ...
      t_info.abbrev = g_strdup ((xchar_t *) &tz_abbrs[info.tt_abbrind]);
...
find_relative_date (TimeZoneDate *buffer)
{
  ...
  if (buffer->mon == 13 || buffer->mon == 14) /* Julian Date */
    ...
    {
      xuint_t days;
      xuint_t days_in_month = g_date_get_days_in_month (buffer->mon, buffer->year);
...
{
  xchar_t *tzstring = g_strndup (footer + 1, footerlen - 2);
  GTimeZone *footertz = NULL;
 ...
 **/
GTimeZone *
g_time_zone_new (const xchar_t *identifier)
{
 ...
 */
GTimeZone *
g_time_zone_new_identifier (const xchar_t *identifier)
{
  GTimeZone *tz = NULL;
  TimeZoneRule *rules;
  xint_t rules_num;
