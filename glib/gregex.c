 ...
 *
 * The xregex_*() functions implement regular
 * expression pattern matching using syntax and semantics similar to
 * Perl regular expression.
 *
 * Some functions accept a @start_position argument, setting it differs
 * from just passing over a shortened string and setting %XREGEX_MATCH_NOTBOL
 ...
 *
 * Note that, unless you set the %XREGEX_RAW flag, all the strings passed
 * to these functions must be encoded in UTF-8. The lengths and the positions
 * inside the strings are in bytes and not in characters, so, for instance,
 * "\xc3\xa0" (i.e. "à") is two bytes long but it is treated as a
 * single character. If you set %XREGEX_RAW the strings can be non-valid
 ...
 * with %XREGEX_NEWLINE_CR, %XREGEX_NEWLINE_LF and %XREGEX_NEWLINE_CRLF
 * compile options, and with %XREGEX_MATCH_NEWLINE_ANY,
 * %XREGEX_MATCH_NEWLINE_CR, %XREGEX_MATCH_NEWLINE_LF and
 * %XREGEX_MATCH_NEWLINE_CRLF match options. These settings are also
 * relevant when compiling a pattern if %XREGEX_EXTENDED is set, and an
 * unescaped "#" outside a character class is encountered. This indicates
 * a comment that lasts until after the next newline.
 *
 * When setting the %XREGEX_JAVASCRIPT_COMPAT flag, pattern syntax and pattern
 ...
 *
 * Creating and manipulating the same #xregex_t structure from different
 * threads is not a problem as #xregex_t does not modify its internal
 * state between creation and destruction, on the other hand #xmatch_info_t
 * is not threadsafe.
 *
...
/* Mask of all the possible values for xregex_compile_flags_t. */
#define XREGEX_COMPILE_MASK (XREGEX_CASELESS          | \
                              XREGEX_MULTILINE         | \
                              XREGEX_DOTALL            | \
                              XREGEX_EXTENDED          | \
                              XREGEX_ANCHORED          | \
                              XREGEX_DOLLAR_ENDONLY    | \
                              XREGEX_UNGREEDY          | \
                              XREGEX_RAW               | \
                              XREGEX_NO_AUTO_CAPTURE   | \
                              XREGEX_OPTIMIZE          | \
                              XREGEX_FIRSTLINE         | \
                              XREGEX_DUPNAMES          | \
                              XREGEX_NEWLINE_CR        | \
                              XREGEX_NEWLINE_LF        | \
                              XREGEX_NEWLINE_CRLF      | \
                              XREGEX_NEWLINE_ANYCRLF   | \
                              XREGEX_BSR_ANYCRLF       | \
                              XREGEX_JAVASCRIPT_COMPAT)

/* Mask of all xregex_compile_flags_t values that are (not) passed trough to PCRE */
#define XREGEX_COMPILE_PCRE_MASK (XREGEX_COMPILE_MASK & ~XREGEX_COMPILE_NONPCRE_MASK)
#define XREGEX_COMPILE_NONPCRE_MASK (XREGEX_RAW              | \
                                      XREGEX_OPTIMIZE)

/* Mask of all the possible values for xregex_match_flags_t. */
#define XREGEX_MATCH_MASK (XREGEX_MATCH_ANCHORED         | \
                            XREGEX_MATCH_NOTBOL           | \
                            XREGEX_MATCH_NOTEOL           | \
                            XREGEX_MATCH_NOTEMPTY         | \
                            XREGEX_MATCH_PARTIAL          | \
                            XREGEX_MATCH_NEWLINE_CR       | \
                            XREGEX_MATCH_NEWLINE_LF       | \
                            XREGEX_MATCH_NEWLINE_CRLF     | \
                            XREGEX_MATCH_NEWLINE_ANY      | \
                            XREGEX_MATCH_NEWLINE_ANYCRLF  | \
                            XREGEX_MATCH_BSR_ANYCRLF      | \
                            XREGEX_MATCH_BSR_ANY          | \
                            XREGEX_MATCH_PARTIAL_SOFT     | \
                            XREGEX_MATCH_PARTIAL_HARD     | \
                            XREGEX_MATCH_NOTEMPTY_ATSTART)

/* we rely on these flags having the same values */
G_STATIC_ASSERT (XREGEX_CASELESS          == PCRE_CASELESS);
G_STATIC_ASSERT (XREGEX_MULTILINE         == PCRE_MULTILINE);
G_STATIC_ASSERT (XREGEX_DOTALL            == PCRE_DOTALL);
G_STATIC_ASSERT (XREGEX_EXTENDED          == PCRE_EXTENDED);
G_STATIC_ASSERT (XREGEX_ANCHORED          == PCRE_ANCHORED);
G_STATIC_ASSERT (XREGEX_DOLLAR_ENDONLY    == PCRE_DOLLAR_ENDONLY);
G_STATIC_ASSERT (XREGEX_UNGREEDY          == PCRE_UNGREEDY);
G_STATIC_ASSERT (XREGEX_NO_AUTO_CAPTURE   == PCRE_NO_AUTO_CAPTURE);
G_STATIC_ASSERT (XREGEX_FIRSTLINE         == PCRE_FIRSTLINE);
G_STATIC_ASSERT (XREGEX_DUPNAMES          == PCRE_DUPNAMES);
G_STATIC_ASSERT (XREGEX_NEWLINE_CR        == PCRE_NEWLINE_CR);
G_STATIC_ASSERT (XREGEX_NEWLINE_LF        == PCRE_NEWLINE_LF);
G_STATIC_ASSERT (XREGEX_NEWLINE_CRLF      == PCRE_NEWLINE_CRLF);
G_STATIC_ASSERT (XREGEX_NEWLINE_ANYCRLF   == PCRE_NEWLINE_ANYCRLF);
G_STATIC_ASSERT (XREGEX_BSR_ANYCRLF       == PCRE_BSR_ANYCRLF);
G_STATIC_ASSERT (XREGEX_JAVASCRIPT_COMPAT == PCRE_JAVASCRIPT_COMPAT);

G_STATIC_ASSERT (XREGEX_MATCH_ANCHORED         == PCRE_ANCHORED);
G_STATIC_ASSERT (XREGEX_MATCH_NOTBOL           == PCRE_NOTBOL);
G_STATIC_ASSERT (XREGEX_MATCH_NOTEOL           == PCRE_NOTEOL);
G_STATIC_ASSERT (XREGEX_MATCH_NOTEMPTY         == PCRE_NOTEMPTY);
G_STATIC_ASSERT (XREGEX_MATCH_PARTIAL          == PCRE_PARTIAL);
G_STATIC_ASSERT (XREGEX_MATCH_NEWLINE_CR       == PCRE_NEWLINE_CR);
G_STATIC_ASSERT (XREGEX_MATCH_NEWLINE_LF       == PCRE_NEWLINE_LF);
G_STATIC_ASSERT (XREGEX_MATCH_NEWLINE_CRLF     == PCRE_NEWLINE_CRLF);
G_STATIC_ASSERT (XREGEX_MATCH_NEWLINE_ANY      == PCRE_NEWLINE_ANY);
G_STATIC_ASSERT (XREGEX_MATCH_NEWLINE_ANYCRLF  == PCRE_NEWLINE_ANYCRLF);
G_STATIC_ASSERT (XREGEX_MATCH_BSR_ANYCRLF      == PCRE_BSR_ANYCRLF);
G_STATIC_ASSERT (XREGEX_MATCH_BSR_ANY          == PCRE_BSR_UNICODE);
G_STATIC_ASSERT (XREGEX_MATCH_PARTIAL_SOFT     == PCRE_PARTIAL_SOFT);
G_STATIC_ASSERT (XREGEX_MATCH_PARTIAL_HARD     == PCRE_PARTIAL_HARD);
G_STATIC_ASSERT (XREGEX_MATCH_NOTEMPTY_ATSTART == PCRE_NOTEMPTY_ATSTART);

/* These PCRE flags are unused or not exposed publicly in GRegexFlags, so
 * it should be ok to reuse them for different things.
 */
G_STATIC_ASSERT (XREGEX_OPTIMIZE          == PCRE_NO_UTF8_CHECK);
G_STATIC_ASSERT (XREGEX_RAW               == PCRE_UTF8);

/* if the string is in UTF-8 use xutf8_ functions, else use
 * use just +/- 1. */
#define NEXT_CHAR(re, s) (((re)->compile_opts & XREGEX_RAW) ? \
                                ((s) + 1) : \
                                xutf8_next_char (s))
#define PREV_CHAR(re, s) (((re)->compile_opts & XREGEX_RAW) ? \
                                ((s) - 1) : \
                                xutf8_prev_char (s))

struct _xmatch_info
{
  xint_t ref_count;               /* the ref count (atomic) */
  xregex_t *regex;                /* the regex */
  xregex_match_flags_t match_opts;  /* options used at match time on the regex */
  xint_t matches;                 /* number of matching sub patterns, guaranteed to be <= (n_subpatterns + 1) if doing a single match (rather than matching all) */
  xint_t n_subpatterns;           /* total number of sub patterns in the regex */
  xint_t pos;                     /* position in the string where last match left off */
  xint_t  n_offsets;              /* number of offsets */
  xint_t *offsets;                /* array of offsets paired 0,1 ; 2,3 ; 3,4 etc */
  xint_t *workspace;              /* workspace for pcre_dfa_exec() */
  xint_t n_workspace;             /* number of workspace elements */
  const xchar_t *string;          /* string passed to the match function */
  xssize_t string_len;            /* length of string, in bytes */
};

struct _xregex
{
  xint_t ref_count;               /* the ref count for the immutable part (atomic) */
  xchar_t *pattern;               /* the pattern */
  pcre *pcre_re;                /* compiled form of the pattern */
  xregex_compile_flags_t compile_opts;      /* options used at compile time on the pattern */
  xregex_match_flags_t match_opts;  /* options used at match time on the regex */
  pcre_extra *extra;            /* data stored when XREGEX_OPTIMIZE is used */
};
...
static xboolean_t  interpolation_list_needs_match (xlist_t *list);
static xboolean_t  interpolate_replacement        (const xmatch_info_t *match_info,
                                                 xstring_t *result,
                                                 xpointer_t data);
static xlist_t    *split_replacement              (const xchar_t *replacement,
                                                 xerror_t **error);
static void      free_interpolation_data        (InterpolationData *data);


static const xchar_t *
match_error (xint_t errcode)
{
  switch (errcode)
    {
    ...
    case PCRE_ERROR_NULL:
      /* NULL argument, this should not happen in xregex_t */
    ...
    case PCRE_ERROR_BADCOUNT:
      /* negative ovecsize, this should not happen in xregex_t */
    ...
    case PCRE_ERROR_DFA_UMLIMIT:
      /* the match_field field is not used in xregex_t */
      break;
...
static void
translate_compile_error (xint_t *errcode, const xchar_t **errmsg)
{
   ...
   * XREGEX_ERROR_COMPILE error code in errcode and keep the
  ...
  *errcode += 100;
  ...
  switch (*errcode)
    ...
    case 124: /* "unrecognized character after (?<\0 */
      *errcode = XREGEX_ERROR_UNRECOGNIZED_CHARACTER;
      *errmsg = _("unrecognized character after (?<");
      break;
    case XREGEX_ERROR_VARIABLE_LENGTH_LOOKBEHIND:
      *errmsg = _("lookbehind assertion is not fixed length");
      break;
    case XREGEX_ERROR_MALFORMED_CONDITION:
      *errmsg = _("malformed number or name after (?(");
      break;
    case XREGEX_ERROR_TOO_MANY_CONDITIONAL_BRANCHES:
      *errmsg = _("conditional group contains more than two branches");
      break;
    case XREGEX_ERROR_ASSERTION_EXPECTED:
    ...
    case 129:
      ...
      *errmsg = _("(?R or (?[+-]digits must be followed by )");
      break;
    case XREGEX_ERROR_UNKNOWN_POSIX_CLASS_NAME:
      *errmsg = _("unknown POSIX class name");
      break;
    case XREGEX_ERROR_POSIX_COLLATING_ELEMENTS_NOT_SUPPORTED:
      *errmsg = _("POSIX collating elements are not supported");
      break;
    case XREGEX_ERROR_HEX_CODE_TOO_LARGE:
      *errmsg = _("character value in \\x{...} sequence is too large");
      break;
    case XREGEX_ERROR_INVALID_CONDITION:
      *errmsg = _("invalid condition (?(0)");
      break;
    case XREGEX_ERROR_SINGLE_BYTE_MATCH_IN_LOOKBEHIND:
    ...
    case 137: /* PCRE does not support \\L, \\l, \\N{name}, \\U, or \\u\0 */
       ...
       */
      *errcode = XREGEX_ERROR_UNRECOGNIZED_ESCAPE;
      *errmsg = _("escapes \\L, \\l, \\N{name}, \\U, and \\u are not supported");
      break;
    case XREGEX_ERROR_INFINITE_LOOP:
    ...
    case 141: /* unrecognized character after (?P\0 */
      ...
      *errmsg = _("unrecognized character after (?P");
      break;
    case XREGEX_ERROR_MISSING_SUBPATTERN_NAME_TERMINATOR:
      *errmsg = _("missing terminator in subpattern name");
      break;
    case XREGEX_ERROR_DUPLICATE_SUBPATTERN_NAME:
      *errmsg = _("two named subpatterns have the same name");
      break;
    case XREGEX_ERROR_MALFORMED_PROPERTY:
      *errmsg = _("malformed \\P or \\p sequence");
      break;
    case XREGEX_ERROR_UNKNOWN_PROPERTY:
      *errmsg = _("unknown property name after \\P or \\p");
      break;
    case XREGEX_ERROR_SUBPATTERN_NAME_TOO_LONG:
      *errmsg = _("subpattern name is too long (maximum 32 characters)");
      break;
    case XREGEX_ERROR_TOO_MANY_SUBPATTERNS:
      *errmsg = _("too many named subpatterns (maximum 10,000)");
      break;
    case XREGEX_ERROR_INVALID_OCTAL_VALUE:
...
static xmatch_info_t *
match_info_new (const xregex_t *regex,
                const xchar_t  *string,
                xint_t          string_len,
                xint_t          start_position,
                xint_t          match_options,
                xboolean_t      is_dfa)
{
  ...
  match_info = g_new0 (xmatch_info_t, 1);
  match_info->ref_count = 1;
  match_info->regex = xregex_ref ((xregex_t *)regex);
  ...
  if (is_dfa)
    {
       ...
       * enough xregex_match_all_full() will expand them. */
      match_info->n_offsets = 24;
      match_info->n_workspace = 100;
      match_info->workspace = g_new (xint_t, match_info->n_workspace);
    }
  else
    {
  ...
  match_info->offsets = g_new0 (xint_t, match_info->n_offsets);
...
/**
 * xmatch_info_get_regex:
 * @match_info: a #xmatch_info_t
 *
 * Returns #xregex_t object used in @match_info. It belongs to Glib
 * and must not be freed. Use xregex_ref() if you need to keep it
 * after you free @match_info object.
 *
 * Returns: #xregex_t object used in @match_info
 *
 * Since: 2.14
 */
xregex_t *
...
/**
 * xmatch_info_get_string:
 * @match_info: a #xmatch_info_t
 *
 * Returns the string searched with @match_info. This is the
 * string passed to xregex_match() or xregex_replace() so
 ...
 */
void
xmatch_info_unref (xmatch_info_t *match_info)
{
  if (g_atomic_int_dec_and_test (&match_info->ref_count))
    {
      xregex_unref (match_info->regex);
...
/**
 * xmatch_info_free:
 * @match_info: (nullable): a #xmatch_info_t, or %NULL
 *
 * If @match_info is not %NULL, calls xmatch_info_unref(); otherwise does
 * nothing.
 *
 * Since: 2.14
 */
void
xmatch_info_free (xmatch_info_t *match_info)
{
  if (match_info == NULL)
    return;
...
/**
 * xmatch_info_next:
 * @match_info: a #xmatch_info_t structure
 * @error: location to store the error occurring, or %NULL to ignore errors
 *
 * Scans for the next match using the same parameters of the previous
 * call to xregex_match_full() or xregex_match() that returned
 * @match_info.
 *
 ...
 */
xboolean_t
xmatch_info_next (xmatch_info_t  *match_info,
                   xerror_t     **error)
{
  ...
  if (IS_PCRE_ERROR (match_info->matches))
    {
      g_set_error (error, XREGEX_ERROR, XREGEX_ERROR_MATCH,
  ...
  if (match_info->matches >= 0 &&
      ...
      prev_match_end == match_info->offsets[1])
    {
      ...
      return xmatch_info_next (match_info, error);
    }
 ...
 * using xregex_match_all() or xregex_match_all_full(), the retrieved
...
/**
 * xmatch_info_is_partial_match:
 * @match_info: a #xmatch_info_t structure
 *
 * Usually if the string passed to xregex_match*() matches as far as
 ...
 *
 * xregex_t supports the concept of partial matching by means of the
 * %XREGEX_MATCH_PARTIAL_SOFT and %XREGEX_MATCH_PARTIAL_HARD flags.
 * When they are used, the return code for
 * xregex_match() or xregex_match_full() is, as usual, %TRUE
 * for a complete match, %FALSE otherwise. But, when these functions
 * return %FALSE, you can check if the match was partial calling
 * xmatch_info_is_partial_match().
 *
 * The difference between %XREGEX_MATCH_PARTIAL_SOFT and
 * %XREGEX_MATCH_PARTIAL_HARD is that when a partial match is encountered
 * with %XREGEX_MATCH_PARTIAL_SOFT, matching continues to search for a
 * possible complete match, while with %XREGEX_MATCH_PARTIAL_HARD matching
 * stops at the partial match.
 * When both %XREGEX_MATCH_PARTIAL_SOFT and %XREGEX_MATCH_PARTIAL_HARD
 ...
 * xregex_replace().
 *
 * The @string_to_expand must be UTF-8 encoded even if %XREGEX_RAW was
 * passed to xregex_new().
 *
 ...
 * Use xregex_check_replacement() to find out whether @string_to_expand
 * contains references.
 *
 ...
 */
xchar_t *
xmatch_info_expand_references (const xmatch_info_t  *match_info,
  ...
  if (!match_info && interpolation_list_needs_match (list))
    {
      g_critical ("String '%s' contains references to the match, can't "
                  "expand references without xmatch_info_t object",
                  string_to_expand);
      return NULL;
    }

  result = xstring_sized_new (strlen (string_to_expand));
  interpolate_replacement (match_info, result, list);

  xlist_free_full (list, (xdestroy_notify_t) free_interpolation_data);
...
/**
 * xmatch_info_fetch:
 * @match_info: #xmatch_info_t structure
 ...
 * xregex_match_all() or xregex_match_all_full(), the retrieved
 ...
 */
xchar_t *
xmatch_info_fetch (const xmatch_info_t *match_info,
                    xint_t              match_num)
{
  ...
  if (!xmatch_info_fetch_pos (match_info, match_num, &start, &end))
    match = NULL;
  else if (start == -1)
    match = xstrdup ("");
  else
    match = xstrndup (&match_info->string[start], end - start);

  return match;
}

/**
 * xmatch_info_fetch_pos:
 * @match_info: #xmatch_info_t structure
 * @match_num: number of the sub expression
 ...
 */
xboolean_t
xmatch_info_fetch_pos (const xmatch_info_t *match_info,
                        xint_t              match_num,
                        xint_t             *start_pos,
                        xint_t             *end_pos)
{
   ...
   * (xregex_match_all()), also compare against the number of matches */
 ...
 */
static xint_t
get_matched_substring_number (const xmatch_info_t *match_info,
                              const xchar_t      *name)
{
  xint_t entrysize;
  xchar_t *first, *last;
 ...
 */
xchar_t *
xmatch_info_fetch_named (const xmatch_info_t *match_info,
                          const xchar_t      *name)
{
  ...
  if (num < 0)
    ...
    return xmatch_info_fetch (match_info, num);
}

/**
 * xmatch_info_fetch_named_pos:
 * @match_info: #xmatch_info_t structure
 * @name: name of the subexpression
 * @start_pos: (out) (optional): pointer to location where to store
 *     the start position, or %NULL
 ...
 */
xboolean_t
xmatch_info_fetch_named_pos (const xmatch_info_t *match_info,
                              const xchar_t      *name,
                              xint_t             *start_pos,
                              xint_t             *end_pos)
{
  ...
  return xmatch_info_fetch_pos (match_info, num, start_pos, end_pos);
}

/**
 * xmatch_info_fetch_all:
 * @match_info: a #xmatch_info_t structure
 *
 * Bundles up pointers to each of the matching substrings from a match
 * and stores them in an array of xchar_t pointers. The first element in
 ...
 * so you cannot call this function after freeing the string.
 *
 * Returns: (transfer full): a %NULL-terminated array of xchar_t *
 *     pointers.  It must be freed using xstrfreev(). If the previous
 *     match failed %NULL is returned
 *
 * Since: 2.14
 */
xchar_t **
xmatch_info_fetch_all (const xmatch_info_t *match_info)
{
  /* we cannot use pcre_get_substring_list() because the returned value
   * isn't suitable for xstrfreev(). */
  xchar_t **result;
  xint_t i;
  ...
  result = g_new (xchar_t *, match_info->matches + 1);
  for (i = 0; i < match_info->matches; i++)
    result[i] = xmatch_info_fetch (match_info, i);
  result[i] = NULL;
...
G_DEFINE_QUARK (g-regex-error-quark, xregex_error)
 ...
 */
static pcre *regex_compile (const xchar_t         *pattern,
                            xregex_compile_flags_t   compile_options,
                            xregex_compile_flags_t  *compile_options_out,
 ...
 * the initial setup of the #xregex_t structure.
 *
 * Returns: (nullable): a #xregex_t structure or %NULL if an error occurred. Call
 *   xregex_unref() when you are done with it
 *
 * Since: 2.14
 */
xregex_t *
xregex_new (const xchar_t         *pattern,
             xregex_compile_flags_t   compile_options,
             xregex_match_flags_t     match_options,
             xerror_t             **error)
{
  ...
  if (G_UNLIKELY (initialised != 1))
    {
      g_set_error_literal (error, XREGEX_ERROR, XREGEX_ERROR_COMPILE,
                           _("PCRE library is compiled with incompatible options"));
      return NULL;
    }

  /* XREGEX_OPTIMIZE has the same numeric value of PCRE_NO_UTF8_CHECK,
   * as we do not need to wrap PCRE_NO_UTF8_CHECK. */
  if (compile_options & XREGEX_OPTIMIZE)
    optimize = TRUE;
  ...
  regex = g_new0 (xregex_t, 1);
  regex->ref_count = 1;
  regex->pattern = xstrdup (pattern);
  regex->pcre_re = re;
  ...
  if (optimize)
    {
      ...
      if (errmsg != NULL)
        {
          xerror_t *tmp_error = xerror_new (XREGEX_ERROR,
...
static pcre *
regex_compile (const xchar_t         *pattern,
               xregex_compile_flags_t   compile_options,
               xregex_compile_flags_t  *compile_options_out,
               xregex_match_flags_t    *match_options,
               xerror_t             **error)
{
  ...
  xint_t erroffset;
  xint_t errcode;
  xregex_compile_flags_t nonpcre_compile_options;
  unsigned long int pcre_compile_options;

  nonpcre_compile_options = compile_options & XREGEX_COMPILE_NONPCRE_MASK;

  /* In xregex_t the string are, by default, UTF-8 encoded. PCRE
   * instead uses UTF-8 only if required with PCRE_UTF8. */
  if (compile_options & XREGEX_RAW)
    {
      /* disable utf-8 */
      compile_options &= ~XREGEX_RAW;
    }
  else
    {
      /* enable utf-8 */
  ...
  if (!(compile_options & XREGEX_NEWLINE_CR) &&
      !(compile_options & XREGEX_NEWLINE_LF))
    {
   ...
   */
  if (~compile_options & XREGEX_BSR_ANYCRLF)
  ...
  if (re == NULL)
    {
      ...
      erroffset = xutf8_pointer_to_offset (pattern, &pattern[erroffset]);

      tmp_error = xerror_new (XREGEX_ERROR, errcode,
  ...
  compile_options = pcre_compile_options & XREGEX_COMPILE_PCRE_MASK;
  ...
  if (!(compile_options & XREGEX_DUPNAMES))
    {
      xboolean_t jchanged = FALSE;
      pcre_fullinfo (re, NULL, PCRE_INFO_JCHANGED, &jchanged);
      if (jchanged)
        compile_options |= XREGEX_DUPNAMES;
    }
...
/**
 * xregex_get_pattern:
 * @regex: a #xregex_t structure
 *
 * Gets the pattern string associated with @regex, i.e. a copy of
 * the string passed to xregex_new().
 *
 ...
 */
xint_t
xregex_get_max_backref (const xregex_t *regex)
{
  xint_t value;
 ...
 */
xint_t
xregex_get_capture_count (const xregex_t *regex)
{
  xint_t value;
 ...
 */
xboolean_t
xregex_get_has_cr_or_lf (const xregex_t *regex)
{
  xint_t value;
 ...
 */
xint_t
xregex_get_max_lookbehind (const xregex_t *regex)
{
  xint_t max_lookbehind;
 ...
 *
 * Returns: flags from #xregex_compile_flags_t
 *
 * Since: 2.26
 */
...
/**
 * xregex_get_match_flags:
 * @regex: a #xregex_t
 *
 * Returns the match options that @regex was created with.
 *
 * Returns: flags from #xregex_match_flags_t
 *
 * Since: 2.26
 */
 ...
 *
 * This function is equivalent to xregex_match() but it does not
 * require to compile the pattern with xregex_new(), avoiding some
 * lines of code when you need just to do a match without extracting
 * substrings, capture counts, and so on.
 *
 * If this function is to be called on the same @pattern more than
 * once, it's more efficient to compile the pattern once with
 * xregex_new() and then use xregex_match().
 *
 ...
 */
xboolean_t
xregex_match_simple (const xchar_t        *pattern,
                      const xchar_t        *string,
                      xregex_compile_flags_t  compile_options,
                      xregex_match_flags_t    match_options)
{
  ...
  regex = xregex_new (pattern, compile_options, 0, NULL);
  if (!regex)
    return FALSE;
  result = xregex_match_full (regex, string, -1, 0, match_options, NULL, NULL);
  xregex_unref (regex);
  return result;
}
...
/**
 * xregex_match:
 * @regex: a #xregex_t structure from xregex_new()
 * @string: the string to scan for matches
 * @match_options: match options
 * @match_info: (out) (optional): pointer to location where to store
 *     the #xmatch_info_t, or %NULL if you do not need it
 *
 * Scans for a match in @string for the pattern in @regex.
 * The @match_options are combined with the match options specified
 * when the @regex structure was created, letting you have more
 * flexibility in reusing #xregex_t structures.
 *
 * Unless %XREGEX_RAW is specified in the options, @string must be valid UTF-8.
 *
 * A #xmatch_info_t structure, used to get information on the match,
 * is stored in @match_info if not %NULL. Note that if @match_info
 * is not %NULL then it is created even if the function returns %FALSE,
 * i.e. you must free it regardless if regular expression actually matched.
 *
 * To retrieve all the non-overlapping matches of the pattern in
 * string you can use xmatch_info_next().
 *
 * |[<!-- language="C" -->
 * static void
 * print_uppercase_words (const xchar_t *string)
 * {
 *   // Print all uppercase-only words.
 *   xregex_t *regex;
 *   xmatch_info_t *match_info;
 *
 *   regex = xregex_new ("[A-Z]+", 0, 0, NULL);
 *   xregex_match (regex, string, 0, &match_info);
 *   while (xmatch_info_matches (match_info))
 *     {
 *       xchar_t *word = xmatch_info_fetch (match_info, 0);
 *       g_print ("Found: %s\n", word);
 *       g_free (word);
 ...
 * ]|
 *
 * @string is not copied and is used in #xmatch_info_t internally. If
 * you use any #xmatch_info_t method (except xmatch_info_free()) after
 ...
 */
xboolean_t
xregex_match (const xregex_t      *regex,
               const xchar_t       *string,
               xregex_match_flags_t   match_options,
               xmatch_info_t       **match_info)
{
  return xregex_match_full (regex, string, -1, 0, match_options,
                             match_info, NULL);
}
 ...
 * string and setting %XREGEX_MATCH_NOTBOL in the case of a pattern
 * that begins with any kind of lookbehind assertion, such as "\b".
 *
 * Unless %XREGEX_RAW is specified in the options, @string must be valid UTF-8.
 *
 * A #xmatch_info_t structure, used to get information on the match, is
 ...
 *
 *   regex = xregex_new ("[A-Z]+", 0, 0, NULL);
 *   xregex_match_full (regex, string, -1, 0, 0, &match_info, &error);
 *   while (xmatch_info_matches (match_info))
 *     {
 *       xchar_t *word = xmatch_info_fetch (match_info, 0);
 *       g_print ("Found: %s\n", word);
 *       g_free (word);
 *       xmatch_info_next (match_info, &error);
 *     }
 *   xmatch_info_free (match_info);
 *   xregex_unref (regex);
 *   if (error != NULL)
 ...
 */
xboolean_t
xregex_match_full (const xregex_t      *regex,
                    const xchar_t       *string,
                    xssize_t             string_len,
                    xint_t               start_position,
                    xregex_match_flags_t   match_options,
                    ...
                    xerror_t           **error)
{
  ...
  info = match_info_new (regex, string, string_len, start_position,
                         match_options, FALSE);
  match_ok = xmatch_info_next (info, error);
  if (match_info != NULL)
    *match_info = info;
  else
 ...
 * For more documentation see xregex_match_all_full().
 *
 * A #xmatch_info_t structure, used to get information on the match, is
 * stored in @match_info if not %NULL. Note that if @match_info is
 * not %NULL then it is created even if the function returns %FALSE,
 ...
 */
xboolean_t
xregex_match_all (const xregex_t      *regex,
                   const xchar_t       *string,
                   xregex_match_flags_t   match_options,
                   xmatch_info_t       **match_info)
{
  return xregex_match_all_full (regex, string, -1, 0, match_options,
                                 match_info, NULL);
}

/**
 * xregex_match_all_full:
 * @regex: a #xregex_t structure from xregex_new()
 * @string: (array length=string_len): the string to scan for matches
 * @string_len: the length of @string, in bytes, or -1 if @string is nul-terminated
 * @start_position: starting index of the string to match, in bytes
 ...
 * xmatch_info_get_match_count(). To obtain the matched strings and
 * their position you can use, respectively, xmatch_info_fetch() and
 * xmatch_info_fetch_pos(). Note that the strings are returned in
 ...
 */
xboolean_t
xregex_match_all_full (const xregex_t      *regex,
                        const xchar_t       *string,
                        xssize_t             string_len,
                        xint_t               start_position,
                        xregex_match_flags_t   match_options,
                        ...
                        xerror_t           **error)
{
      ...
      if (info->matches == PCRE_ERROR_DFA_WSSIZE)
        {
          ...
          info->workspace = g_realloc (info->workspace,
                                       info->n_workspace * sizeof (xint_t));
          done = FALSE;
        }
  ...
  /* set info->pos to -1 so that a call to xmatch_info_next() fails. */
  info->pos = -1;
...
/**
 * xregex_get_string_number:
 * @regex: #xregex_t structure
 * @name: name of the subexpression
 *
 ...
 */
xint_t
xregex_get_string_number (const xregex_t *regex,
                           const xchar_t  *name)
{
  xint_t num;
 ...
 *
 * This function is equivalent to xregex_split() but it does
 * not require to compile the pattern with xregex_new(), avoiding
 * some lines of code when you need just to do a split without
 * extracting substrings, capture counts, and so on.
 *
 * If this function is to be called on the same @pattern more than
 * once, it's more efficient to compile the pattern once with
 * xregex_new() and then use xregex_split().
 *
 ...
 **/
xchar_t **
xregex_split_simple (const xchar_t        *pattern,
                      const xchar_t        *string,
                      xregex_compile_flags_t  compile_options,
                      xregex_match_flags_t    match_options)
{
  ...
  result = xregex_split_full (regex, string, -1, 0, match_options, 0, NULL);
  xregex_unref (regex);
  return result;
}
 ...
 * "a", "b" and "c".
 *
 * Returns: (transfer full): a %NULL-terminated xchar_t ** array. Free
 * it using xstrfreev()
 *
 * Since: 2.14
 **/
xchar_t **
xregex_split (const xregex_t     *regex,
               const xchar_t      *string,
               xregex_match_flags_t  match_options)
{
  return xregex_split_full (regex, string, -1, 0,
 ...
 **/
xchar_t **
xregex_split_full (const xregex_t      *regex,
                    const xchar_t       *string,
                    xssize_t             string_len,
                    xint_t               start_position,
                    xregex_match_flags_t   match_options,
                    ...
                    xerror_t           **error)
{
  ...
  xint_t last_separator_end;
  /* was the last match 0 bytes long? */
  xboolean_t last_match_is_empty;
  ...
  if (max_tokens == 1)
    {
      string_list = g_new0 (xchar_t *, 2);
      string_list[0] = xstrndup (&string[start_position],
  ...
  match_ok = xregex_match_full (regex, string, string_len, start_position,
  ...
  while (tmp_error == NULL)
    {
      if (match_ok)
          ...
          if (last_separator_end != match_info->offsets[1])
            {
              ...
              token = xstrndup (string + last_separator_end,
                                 match_info->offsets[0] - last_separator_end);
              list = xlist_prepend (list, token);
              token_count++;
              ...
              if (match_count > 1)
                {
                  for (i = 1; i < match_count; i++)
                    list = xlist_prepend (list, xmatch_info_fetch (match_info, i));
                }
            }
        }
      else
        {
          ...
          if (!last_match_is_empty)
            {
              xchar_t *token = xstrndup (string + last_separator_end,
      ...
      if (token_count >= max_tokens - 1)
        {
          ...
          if (string_len > match_info->pos)
            {
              xchar_t *token = xstrndup (string + match_info->pos,
      ...
      if (last_match_is_empty)
        /* if the last match was empty, xmatch_info_next() has moved
      ...
      match_ok = xmatch_info_next (match_info, &tmp_error);
    }
  xmatch_info_free (match_info);
  if (tmp_error != NULL)
    {
  ...
  string_list = g_new (xchar_t *, xlist_length (list) + 1);
  i = 0;
  for (last = xlist_last (list); last; last = xlist_previous (last))
...
static const xchar_t *
expand_escape (const xchar_t        *replacement,
               const xchar_t        *p,
               ...
               xerror_t            **error)
{
  ...
  switch (*p)
    ...
    case 'x':
      ...
      data->text = g_new0 (xchar_t, 8);
      xunichar_to_utf8 (x, data->text);
      break;
    case 'l':
      p++;
    ...
    case '0':
      ...
      if (g_ascii_digit_value (*xutf8_next_char (p)) >= 0)
        {
          base = 8;
          p = xutf8_next_char (p);
        }
 ...
 error:
  ...
  tmp_error = xerror_new (XREGEX_ERROR,
...
static xlist_t *
split_replacement (const xchar_t  *replacement,
                   xerror_t      **error)
{
  xlist_t *list = NULL;
  InterpolationData *data;
  const xchar_t *p, *start;
          ...
          if (*p == '\\' || *p == '\0')
            {
              if (p - start > 0)
                {
                  data = g_new0 (InterpolationData, 1);
                  data->text = xstrndup (start, p - start);
                  data->type = REPL_TYPE_STRING;
                  list = xlist_prepend (list, data);
                }
            }
               ...
               ChangeCase  *change_case)
{
  ...
  if (*change_case == CHANGE_CASE_NONE)
    {
      xstring_append (string, text);
    }
  else if (*change_case & CHANGE_CASE_SINGLE_MASK)
    {
      c = xutf8_get_char (text);
      xstring_append_unichar (string, CHANGE_CASE (c, *change_case));
      xstring_append (string, xutf8_next_char (text));
      ...
      while (*text != '\0')
        {
          c = xutf8_get_char (text);
          xstring_append_unichar (string, CHANGE_CASE (c, *change_case));
          text = xutf8_next_char (text);
        }
    }
}

static xboolean_t
interpolate_replacement (const xmatch_info_t *match_info,
                         xstring_t          *result,
                         xpointer_t          data)
{
  xlist_t *list;
  ...
  for (list = data; list; list = list->next)
    {
      ...
      switch (idata->type)
        ...
        case REPL_TYPE_CHARACTER:
          xstring_append_c (result, CHANGE_CASE (idata->c, change_case));
          if (change_case & CHANGE_CASE_SINGLE_MASK)
            change_case = CHANGE_CASE_NONE;
          break;
        case REPL_TYPE_NUMERIC_REFERENCE:
          match = xmatch_info_fetch (match_info, idata->num);
          if (match)
            {
        ...
        case REPL_TYPE_SYMBOLIC_REFERENCE:
          match = xmatch_info_fetch_named (match_info, idata->text);
          if (match)
            {
              string_append (result, match, &change_case);
              g_free (match);
 ...
 */
static xboolean_t
interpolation_list_needs_match (xlist_t *list)
{
  while (list != NULL)
    {
 ...
 *
 * If you do not need to use backreferences use xregex_replace_literal().
 *
 * The @replacement string must be UTF-8 encoded even if %XREGEX_RAW was
 * passed to xregex_new(). If you want to use not UTF-8 encoded strings
 * you can use xregex_replace_literal().
 *
 * Setting @start_position differs from just passing over a shortened
 * string and setting %XREGEX_MATCH_NOTBOL in the case of a pattern that
 ...
 */
xchar_t *
xregex_replace (const xregex_t      *regex,
                 const xchar_t       *string,
                 xssize_t             string_len,
                 xint_t               start_position,
                 ...
                 xerror_t           **error)
{
  ...
  result = xregex_replace_eval (regex,
...
static xboolean_t
literal_replacement (const xmatch_info_t *match_info,
                     xstring_t          *result,
                     xpointer_t          data)
{
  xstring_append (result, data);
  return FALSE;
}

/**
 * xregex_replace_literal:
 * @regex: a #xregex_t structure
 * @string: (array length=string_len): the string to perform matches against
 * @string_len: the length of @string, in bytes, or -1 if @string is nul-terminated
 * @start_position: starting index of the string to match, in bytes
 ...
 * include backreferences use xregex_replace().
 *
 * Setting @start_position differs from just passing over a
 * shortened string and setting %XREGEX_MATCH_NOTBOL in the
 ...
 */
xchar_t *
xregex_replace_literal (const xregex_t      *regex,
                         const xchar_t       *string,
                         xssize_t             string_len,
                         xint_t               start_position,
                         const xchar_t       *replacement,
 ...
 * that begins with any kind of lookbehind assertion, such as "\b".
 *
 * The following example uses xregex_replace_eval() to replace multiple
 * strings at once:
 * |[<!-- language="C" -->
 * static xboolean_t
 * eval_cb (const xmatch_info_t *info,
 *          xstring_t          *res,
 *          xpointer_t          data)
 * {
 *   xchar_t *match;
 *   xchar_t *r;
 *
 *    match = xmatch_info_fetch (info, 0);
 *    r = xhash_table_lookup ((xhashtable_t *)data, match);
 *    xstring_append (res, r);
 *    g_free (match);
 *
 *    return FALSE;
 * }
 ...
 * xchar_t *res;
 *
 * h = xhash_table_new (xstr_hash, xstr_equal);
 *
 * xhash_table_insert (h, "1", "ONE");
 * xhash_table_insert (h, "2", "TWO");
 * xhash_table_insert (h, "3", "THREE");
 * xhash_table_insert (h, "4", "FOUR");
 *
 * reg = xregex_new ("1|2|3|4", 0, 0, NULL);
 * res = xregex_replace_eval (reg, text, -1, 0, 0, eval_cb, h, NULL);
 ...
 */
xchar_t *
xregex_replace_eval (const xregex_t        *regex,
                      const xchar_t         *string,
                      xssize_t               string_len,
                      xint_t                 start_position,
                      xregex_match_flags_t     match_options,
                      ...
                      xerror_t             **error)
{
  ...
  result = xstring_sized_new (string_len);

  /* run down the string making matches. */
  xregex_match_full (regex, string, string_len, start_position,
                      match_options, &match_info, &tmp_error);
  while (!done && xmatch_info_matches (match_info))
    {
      xstring_append_len (result,
                           string + str_pos,
                           match_info->offsets[0] - str_pos);
      done = (*eval) (match_info, result, user_data);
      str_pos = match_info->offsets[1];
      xmatch_info_next (match_info, &tmp_error);
    }
  xmatch_info_free (match_info);
  if (tmp_error != NULL)
    {
  ...
  xstring_append_len (result, string + str_pos, string_len - str_pos);
  return xstring_free (result, FALSE);
}

/**
 * xregex_check_replacement:
 ...
 * (see xregex_replace()), i.e. that all escape sequences in
 * it are valid.
 *
 ...
 * subpattern) requires valid #xmatch_info_t object.
 *
 ...
 */
xboolean_t
xregex_check_replacement (const xchar_t  *replacement,
 ...
 */
xchar_t *
xregex_escape_nul (const xchar_t *string,
                    xint_t         length)
{
  xstring_t *escaped;
  ...
  escaped = xstring_sized_new (length + 1);
          ...
          if (p != piece_start)
            {
              ...
              xstring_append_len (escaped, piece_start, p - piece_start);
            }
          if ((backslashes & 1) == 0)
            xstring_append_c (escaped, '\\');
          xstring_append_c (escaped, 'x');
          xstring_append_c (escaped, '0');
          xstring_append_c (escaped, '0');
          piece_start = ++p;
          backslashes = 0;
          break;
  ...
  if (piece_start < end)
    xstring_append_len (escaped, piece_start, end - piece_start);

  return xstring_free (escaped, FALSE);
}

/**
 * xregex_escape_string:
 ...
 */
xchar_t *
xregex_escape_string (const xchar_t *string,
                       xint_t         length)
{
  xstring_t *escaped;
