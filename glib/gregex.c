...
static xboolean_t  interpolate_replacement        (const GMatchInfo *match_info,
                                                 GString *result,
                                                 xpointer_t data);
 ...
 */
GRegex *
g_regex_new (const xchar_t         *pattern,
             GRegexCompileFlags   compile_options,
             GRegexMatchFlags     match_options,
             xerror_t             **error)
{
  ...
  if (optimize)
    {
      ...
      if (errmsg != NULL)
        {
          xerror_t *tmp_error = g_error_new (G_REGEX_ERROR,
 ...
 */
xboolean_t
g_regex_match_simple (const xchar_t        *pattern,
                      const xchar_t        *string,
                      GRegexCompileFlags  compile_options,
                      GRegexMatchFlags    match_options)
{
 ...
 *     {
 *       xchar_t *word = g_match_info_fetch (match_info, 0);
 *       g_print ("Found: %s\n", word);
 *       g_free (word);
 ...
 **/
xchar_t **
g_regex_split_simple (const xchar_t        *pattern,
                      const xchar_t        *string,
                      GRegexCompileFlags  compile_options,
                      GRegexMatchFlags    match_options)
{
                    ...
                    xerror_t           **error)
{
      ...
      if (match_ok)
      ...
      else
        {
          ...
          if (!last_match_is_empty)
            {
              xchar_t *token = g_strndup (string + last_separator_end,
      ...
      if (token_count >= max_tokens - 1)
        {
          ...
          if (string_len > match_info->pos)
            {
              xchar_t *token = g_strndup (string + match_info->pos,
  ...
  string_list = g_new (xchar_t *, g_list_length (list) + 1);
  i = 0;
 ...
 */
xboolean_t
g_regex_check_replacement (const xchar_t  *replacement,
 ...
 */
xchar_t *
g_regex_escape_nul (const xchar_t *string,
                    xint_t         length)
{
  GString *escaped;
 ...
 */
xchar_t *
g_regex_escape_string (const xchar_t *string,
                       xint_t         length)
{
  GString *escaped;
