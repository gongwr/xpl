...
GDateTime *__dt = g_date_time_new_local (2009, 10, 24, 0, 0, 0);\
  xchar_t *__p = g_date_time_format (__dt, (f));                  \
...
#define TEST_PRINTF_DATE(y,m,d,f,o)             G_STMT_START {  \
  ...
  xchar_t *p = g_date_time_format (dt, (f));                      \
  xchar_t *o_casefold = g_utf8_casefold (o, -1);                  \
  xchar_t *p_casefold = g_utf8_casefold (p, -1);                  \
...
{
  xchar_t *oldlocale = g_strdup (setlocale (LC_ALL, NULL));
