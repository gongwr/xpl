...
test_new_from_bytes (void)
{
  ...
  g_assert_true (g_bytes_get_data (sub, NULL) == ((xchar_t *)g_bytes_get_data (bytes, NULL)) + 10);
  g_bytes_unref (bytes);
...
test_to_data_different_free_func (void)
{
  ...
  xsize_t size;
  GBytes *bytes;
  xchar_t *sentinel = g_strdup ("hello");
...
test_get_region (void)
{
  ...
  g_assert_true (g_bytes_get_region (bytes, 1, G_MAXSSIZE, ((xsize_t) G_MAXSSIZE) + 1) == NULL);
