...
struct _GMainContext
{
  ...
  GHashTable *sources;              /* xuint_t -> GSource */
...
static void     g_source_iter_init  (GSourceIter   *iter,
				     GMainContext  *context,
				     xboolean_t       may_modify);
static xboolean_t g_source_iter_next  (GSourceIter   *iter,
				     GSource      **source);
static void     g_source_iter_clear (GSourceIter   *iter);

static xboolean_t g_timeout_dispatch (GSource     *source,
				    GSourceFunc  callback,
				    xpointer_t     user_data);
static xboolean_t g_child_watch_prepare  (GSource     *source,
				        xint_t        *timeout);
static xboolean_t g_child_watch_check    (GSource     *source);
static xboolean_t g_child_watch_dispatch (GSource     *source,
					GSourceFunc  callback,
					xpointer_t     user_data);
static void     g_child_watch_finalize (GSource     *source);
#ifdef G_OS_UNIX
static void g_unix_signal_handler (int signum);
static xboolean_t g_unix_signal_watch_prepare  (GSource     *source,
					      xint_t        *timeout);
static xboolean_t g_unix_signal_watch_check    (GSource     *source);
static xboolean_t g_unix_signal_watch_dispatch (GSource     *source,
					      GSourceFunc  callback,
					      xpointer_t     user_data);
static void     g_unix_signal_watch_finalize  (GSource     *source);
#endif
static xboolean_t g_idle_prepare     (GSource     *source,
				    xint_t        *timeout);
static xboolean_t g_idle_check       (GSource     *source);
static xboolean_t g_idle_dispatch    (GSource     *source,
				    GSourceFunc  callback,
				    xpointer_t     user_data);
 ...
 */
GMainContext *
g_main_context_new_with_next_id (xuint_t next_id)
{
		 ...
		 GMainContext *context)
{
  ...
  TRACE (XPL_MAIN_SOURCE_ATTACH (g_source_get_name (source), source, context,
                                  result));
...
g_source_destroy_internal (GSource      *source,
			   ...
			   xboolean_t      have_lock)
{
  TRACE (XPL_MAIN_SOURCE_DESTROY (g_source_get_name (source), source,
                                   context));
...
static void
g_source_callback_ref (xpointer_t cb_data)
{
...
static void
g_source_callback_unref (xpointer_t cb_data)
{
  GSourceCallback *callback = cb_data;
...
static void
g_source_callback_get (xpointer_t     cb_data,
		       GSource     *source,
		       GSourceFunc *func,
		       xpointer_t    *data)
{
 ...
 **/
void
g_source_set_name_by_id (xuint_t           tag,
                         const char     *name)
{
  GSource *source;
 ...
 **/
xboolean_t
g_source_remove_by_user_data (xpointer_t user_data)
{
  GSource *source;
  ...
  for (i = 0; i < context->pending_dispatches->len; i++)
      ...
      if (!SOURCE_DESTROYED (source))
	{
	  ...
	  UNLOCK_CONTEXT (context);
          ...
          TRACE (XPL_MAIN_BEFORE_DISPATCH (g_source_get_name (source), source,
                                            dispatch, callback, user_data));
          need_destroy = !(* dispatch) (source, callback, user_data);
          TRACE (XPL_MAIN_AFTER_DISPATCH (g_source_get_name (source), source,
		      ...
		      xint_t          n_fds)
{
  ...
  while (g_source_iter_next (&iter, &source))
	...
	{
          xboolean_t result;
          xboolean_t (* check) (GSource *source);
 ...
 **/
xboolean_t
g_main_context_iteration (GMainContext *context, xboolean_t may_block)
{
  xboolean_t retval;
      ...
      if (timer_perturb == -1)
        {
          ...
          if (session_bus_address)
            timer_perturb = ABS ((xint_t) g_str_hash (session_bus_address)) % 1000000;
          else
            timer_perturb = 0;
        }
		    ...
		    GDestroyNotify notify)
{
  ...
  TRACE (XPL_TIMEOUT_ADD (source, g_main_context_default (), id, priority, interval, function, data));
		 ...
		 GDestroyNotify notify)
{
  ...
  TRACE (XPL_IDLE_ADD (source, g_main_context_default (), id, priority, function, data));
