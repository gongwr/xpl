 ...
 *
 * xrand_t uses the Mersenne Twister PRNG, which was originally
 ...
 * functions, which will create a globally used #xrand_t and use the
 * according g_rand_* functions internally. Whenever you need a
 * stream of reproducible random numbers, you better create a
 * #xrand_t yourself and use the g_rand_* functions directly, which
 * will also be slightly faster. Initializing a #xrand_t with a
 ...
 * generator Mersenne Twister, as used by #xrand_t. This was necessary,
 ...
 * xrand_t:
 *
 * The xrand_t struct is an opaque data structure. It should only be
...
get_random_version (void)
{
  ...
  static xuint_t random_version;

  if (g_once_init_enter (&initialized))
    {
      const xchar_t *version_string = g_getenv ("G_RANDOM_VERSION");
...
struct _GRand
{
  xuint32_t mt[N]; /* the array for the state vector  */
  xuint_t mti;
};
 ...
 *
 * Returns: the new #xrand_t
 **/
xrand_t*
g_rand_new_with_seed (xuint32_t seed)
{
  xrand_t *rand = g_new0 (xrand_t, 1);
 ...
 */
xrand_t*
g_rand_new_with_seed_array (const xuint32_t *seed,
                            xuint_t          seed_length)
{
  xrand_t *rand = g_new0 (xrand_t, 1);
...
g_rand_new (void)
{
  xuint32_t seed[4];
#ifdef G_OS_UNIX
  static xboolean_t dev_urandom_exists = TRUE;
 ...
 * @rand_: a #xrand_t
 *
 * Frees the memory allocated for the #xrand_t.
 */
void
g_rand_free (xrand_t *rand)
{
 ...
 * @rand_: a #xrand_t
 *
 * Copies a #xrand_t into a new one with the same exact state as before.
 ...
 *
 * Sets the seed for the random number generator #xrand_t to @seed.
 */
void
g_rand_set_seed (xrand_t   *rand,
                 xuint32_t  seed)
{
  g_return_if_fail (rand != NULL);
 ...
 */
void
g_rand_set_seed_array (xrand_t         *rand,
 ...
 * @rand_: a #xrand_t
 *
 * Returns a random #xboolean_t from @rand_.
 * This corresponds to an unbiased coin toss.
 *
 * Returns: a random #xboolean_t
 */
/**
 * g_rand_int:
 * @rand_: a #xrand_t
 *
 * Returns the next random #xuint32_t from @rand_ equally distributed over
 * the range [0..2^32-1].
 *
 * Returns: a random number
 */
xuint32_t
g_rand_int (xrand_t *rand)
{
  xuint32_t y;
                  ...
                  gint32  end)
{
  ...
  switch (get_random_version ())
      ...
      if (dist <= 0x10000L) /* 2^16 */
	{
	  ...
	  xdouble_t double_rand = g_rand_int (rand) *
	  ...
	  if (dist <= 0x80000000u) /* 2^31 */
	    {
	      ...
	      xuint32_t leftover = (0x80000000u % dist) * 2;
 ...
 * @rand_: a #xrand_t
 *
 * Returns the next random #xdouble_t from @rand_ equally distributed over
 * the range [0..1).
 *
 * Returns: a random number
 */
xdouble_t
g_rand_double (xrand_t *rand)
{
  /* We set all 52 bits after the point for this, not only the first
     32. That's why we need two calls to g_rand_int */
  xdouble_t retval = g_rand_int (rand) * G_RAND_DOUBLE_TRANSFORM;
 ...
 */
xdouble_t
g_rand_double_range (xrand_t   *rand,
                     xdouble_t  begin,
                     xdouble_t  end)
{
  xdouble_t r;
 ...
 * g_random_int:
 *
 * Return a random #xuint32_t equally distributed over the range
 * [0..2^32-1].
 *
 * Returns: a random number
 */
 ...
 * g_random_double:
 *
 * Returns a random #xdouble_t equally distributed over the range [0..1).
 *
 * Returns: a random number
 */
xdouble_t
 ...
 * @end: upper open bound of the interval
 *
 * Returns a random #xdouble_t equally distributed over the range
 * [@begin..@end).
 *
 * Returns: a random number
 */
xdouble_t
g_random_double_range (xdouble_t begin,
                       xdouble_t end)
{
  double result;
  G_LOCK (global_random);
 ...
 */
void
g_random_set_seed (xuint32_t seed)
{
  G_LOCK (global_random);
