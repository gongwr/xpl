 ...
 * #xint_t and #glong are different sizes, as the flags are cast
...
static xtype_t my_test_flags_get_type (void);
  ...
  for (i = 0; i < G_N_ELEMENTS (test_flags); i++)
    {
      ...
      xobject_t *test = g_object_new (XTYPE_TEST,
				    "flags", flag_set,
				    NULL);

      g_object_get (test, "flags", &flag_read, NULL);

      /* This check will fail in case of xint_t -> glong conversion
