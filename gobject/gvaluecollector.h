...
G_STMT_START {										\
  ...
  while (*g_vci_collect_format)								\
	...
	case G_VALUE_COLLECT_POINTER:							\
	  g_vci_cvalue->v_pointer = va_arg ((var_args), xpointer_t);				\
	  break;									\
	default:									\
 ...
 * @_value_type: the #xtype_t of the value to skip
 * @var_args: the va_list variable; it may be evaluated multiple times
 *
...
G_STMT_START {										\
  ...
  while (*g_vl_lcopy_format)								\
	...
	case G_VALUE_COLLECT_DOUBLE:							\
	  ...
	  break;									\
	case G_VALUE_COLLECT_POINTER:							\
	  g_vl_cvalue->v_pointer = va_arg ((var_args), xpointer_t);				\
	  break;									\
	default:									\
	  g_assert_not_reached ();							\
	}										\
